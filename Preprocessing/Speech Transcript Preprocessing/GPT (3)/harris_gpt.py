...
def extract_topics(text):
    ...
    try:
        ...
        for chunk in chunks:
            prompt = (
                f"The following are sentences said by Kamala Harris. Extract key topics that align with "
...
df = pd.read_csv('harris_transcripts_cleaned.csv')
...
df.to_csv('harris_transcripts_with_topics (v2).csv', index=False)
