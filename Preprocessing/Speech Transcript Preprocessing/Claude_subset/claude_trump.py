        ...
        for chunk in chunks:
            message = client.messages.create(
                ...
                messages=[
                    {
                        "role": "user",
                        "content": f"""Extract key political topics from this text that match the following categories: {', '.join(VALID_TOPICS)}
                        
                        Text: {chunk}
                        
                        Respond only with most relevant matching topics separated by commas. If no topics are found, respond with 'NaN'."""
                    }
                ]
            )
...
if __name__ == "__main__":
    input_file = 'trump_topic_ryan.csv'
    output_file = 'trump_topics_claude_mini.csv'
