...
def get_transcript_links(base_url, num_pages):
    ...
    for page_num in range(1, num_pages + 1):
        # Handle the format for different pages
        if page_num == 1:
            url = base_url
        else:
            url = f'https://www.rev.com/blog/transcripts/page/{page_num}?s=harris'
...
# Main script
base_url = 'https://www.rev.com/blog/transcripts?s=harris'
links = get_transcript_links(base_url, num_pages=20) 
...
# Save to CSV
if data:
    save_to_csv('kamala_harris_transcripts.csv', data)
